...
- 调用方的请求经Automatic Scene Configuration Kit，由评估配置服务代理通过SAManager按需拉起并访问系统服务层的评估配置服务，实现应用层到系统服务层的进程间通信（IPC）。
...
- 评估期间通知提醒受限，包括无铃声、震动、横幅、实况卡片（包含NFC）等，包含Push通知和系统服务通知；
- 音视频通话应用若未被设置在allowedApps参数中，评估期间打不通；音视频通话应用若已被设置在allowedApps参数中，评估期间只有提醒（如响铃），但无法接听（实况卡片通知提醒受限）；
- 限制转发通知到三方设备（通知系统服务）；
...
- 服务以5秒为最大时间片轮询，剩余不足5秒时按剩余时长收敛；到期判定基于进入评估时记录的到期时间点，轮询抖动不会累积，实际解除时刻相对设定时长仅有毫秒级偏差（线程唤醒与调度延迟）。到期后自动解除管控。
- 监听调用方进程，调用方退出即结束评估。
...
1. **评估时长到期**：生命周期管理模块的倒计时线程以5秒为最大时间片比较当前时间与进入评估时记录的到期时间点（`duration`为0时取默认8小时），剩余不足5秒时按剩余时长收敛；到期后解除管控，由本服务回调`onInterrupted`（`TIMEOUT`，code=2）告知调用方评估被中断。
...
| 时间 | 主要变更 |
|------|----------|
| 2026-08 | 初始开源：提供评估模式配置接口（`begin`、`end`、`isActive`、`getConfiguration`）与评估状态回调（`onBegin`、`onInterrupted`、`onEnd`），支持Kiosk管控、评估时长与自动解除、评估状态持久化 |
| 2026-09 | 对接元能力基础框架进出Kiosk模式接口；新增三方评估应用进程状态监听；回调码枚举`SECURITY_BREACH`更名为`ENV_ANOMALY`；移除anco与低功耗管理相关引用；补充服务与工具类单元测试 |
